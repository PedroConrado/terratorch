...
class CarbonFluxNonGeoDataModule(NonGeoDataModule):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.means = [MEANS["image"][b] for b in bands]
        self.stds = [STDS["image"][b] for b in bands]
        self.bands = bands
        self.train_transform = wrap_in_compose_is_list(train_transform)
        self.val_transform = wrap_in_compose_is_list(val_transform)
        self.test_transform = wrap_in_compose_is_list(test_transform)
        self.aug = Normalize(self.means, self.stds) if aug is None else aug
        self.merra_vars_means = MEANS["merra_vars"]
        self.merra_vars_stds = STDS["merra_vars"]
    ...
    def setup(self, stage: str) -> None:
        if stage in ["fit"]:
            self.train_dataset = self.dataset_class(
                ...
                bands=self.bands,
                merra_means=self.merra_vars_means,
                merra_stds=self.merra_vars_stds,
