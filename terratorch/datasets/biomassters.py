...
class BioMasstersNonGeo(BioMassters):
    ...
    * https://nascetti-a.github.io/BioMasster/
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        target = self._load_target(Path(target_filename))
        target = np.array(target)
        target_norm = (target - self.mask_mean) / self.mask_std
        #target_norm = torch.tensor(target_norm, dtype=torch.float32)
        target_norm = target_norm.transpose(1, 2, 0)
        output["mask"] = target_norm
        if self.transform:
            if len(self.sensors) == 1:
