...
class CarbonFluxNonGeo(NonGeoDataset):
    ...
    def __init__(
        ...
        merra_means: Sequence[float] | None = None,
        merra_stds: Sequence[float] | None = None,
        ...
        if merra_means is None or merra_stds is None:
            msg = "Means and standard deviations for MERRA data must be provided."
            raise ValueError(msg)
        self.merra_means = np.array(merra_means, dtype=np.float32)
        self.merra_stds = np.array(merra_stds, dtype=np.float32)
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        merra_vars, gpp = sample["merra_vars"], sample["gpp"]
        merra_vars_norm = (merra_vars - self.merra_means) / self.merra_stds
        merra_vars_norm = torch.from_numpy(merra_vars_norm).float().reshape(-1, 1, 1)
        gpp_norm = (gpp - self.gpp_mean) / self.gpp_std
        gpp_norm = torch.tensor([gpp_norm], dtype=torch.float32)

        output["merra_vars"] = merra_vars_norm
        output["mask"] = gpp_norm
