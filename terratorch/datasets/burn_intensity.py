...
class BurnIntensityNonGeo(NonGeoDataset):
    ...
    # Define all possible band names and class names
    all_band_names = (
    ...
    def plot(self, sample: dict[str, Tensor], suptitle: str | None = None) -> Any:
        ...
        ax[len(self.time_steps)].imshow(mask, cmap="jet", vmin=0, vmax=self.num_classes - 1)
        ax[len(self.time_steps)].axis("off")
        ax[len(self.time_steps)].set_title("Burn Intensity Mask")

        cmap = plt.get_cmap("jet", self.num_classes)
        legend_handles = [Rectangle((0, 0), 1, 1, color=cmap(i)) for i in range(self.num_classes)]
        legend_labels = self.class_names
        ax[-1].legend(legend_handles, legend_labels, loc="center")
        ax[-1].axis("off")
